...
! ------------------------------------------------------------------------------
    function test_siso_model_fit_least_squares_multi() result(rst)
        ! Arguments
        logical :: rst
        ...
        ! Local Variables
        integer(int32) :: i
        real(real64) :: dt, tmax, p(2), ic(2,2)
        type(dynamic_system_measurement) :: measurements(2)
        procedure(ode), pointer :: fcn
        type(ode_container) :: mdl
        type(runge_kutta_45) :: integrator
...
! EXCITATION 1
        ...
        ! Generate the solution for the system
        ic(1,:) = 0.0d0  ! zero-valued initial conditions
        call interp%initialize(measurements(1)%t, measurements(1)%input)
        info%model = [wn, zeta]
        info%excitation => interp
        mdl%fcn => eom
        call integrator%solve(mdl, measurements(1)%t, ic(1,:), args = info)
        sol = integrator%get_solution()
        measurements(1)%output = sol(:,2)
    ...
    ! EXCITATION 2
        ...
        info%excitation => interp2
        call integrator%clear_buffer()
        call integrator%solve(mdl, measurements(2)%t, ic(2,:), args = info)
        sol = integrator%get_solution()
        measurements(2)%output = sol(:,2)
    ...
    ! --------------------------------------------------------------------------
        ...
        if (.not.assert(wn, p(1), tol * wn)) then
            rst = .false.
            print "(A)", "TEST FAILED: test_siso_model_fit_least_squares_multi -1"
        end if

        if (.not.assert(zeta, p(2), tol * zeta)) then
            rst = .false.
            print "(A)", "TEST FAILED: test_siso_model_fit_least_squares_multi -2"
        end if
    end function
